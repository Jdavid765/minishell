...
/*   Updated: 2026/05/30 20:46:39 by canoduran        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	setup_env(t_all *all, char **env)
{
	...
	while (env[i])
	{
		...
		if (!key || !value)
			return (clear_setup_env(key, value, head), 1);
	...
	if (shlvl_add(&head) || check_path(&head))
		return (clear_setup_env(NULL, NULL, head), 1);
	all->env = head;
	return (0);
}
/*
...
int	create_shlvl(t_env **head)
{
	...
	value = ft_strdup("1");
	if (!value)
		return (free(key), perror("Malloc :"), 1);
	current = ft_node_env(key, value);
	if (!current)
